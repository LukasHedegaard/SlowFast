...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            # Map pytorchvideo names to slowfast
            pre_train_dict = {
                (
                    k.replace("blocks.0.conv.", "s1.pathway0_stem.conv.")
                    .replace("blocks.0.norm.", "s1.pathway0_stem.bn.")
                    .replace("blocks.1.res_blocks.", "s2.pathway0_res")
                    .replace("blocks.2.res_blocks.", "s3.pathway0_res")
                    .replace("blocks.3.res_blocks.", "s4.pathway0_res")
                    .replace("blocks.4.res_blocks.", "s5.pathway0_res")
                    .replace("blocks.5.proj", "head.projection")
                    .replace("branch1_conv", "branch1")
                    .replace("branch1_norm", "branch1_bn")
                    .replace("conv_a", "a")
                    .replace("conv_b", "b")
                    .replace("conv_c", "c")
                    .replace("norm_a", "a_bn")
